...
class ProductAnalyticsBatchCommand extends Command
{

    protected $signature = 'products:analytics {--limit=20 : Number of products to process}';
    ...
    public function handle()
    {
        ...
        $products = \App\Models\Product::where('step', 2)
        ...
        try {
            $batch = Bus::batch(
                $products->map(fn($product) => new ProductAnalyJob($product->id))
            )->name('Save Batch')
                ->onQueue('prices_analysis')
                ->dispatch();
