    ...
    {{ $item->products()->count() }}
                            ...
                            <td class="px-2 py-2 whitespace-nowrap text-end text-sm font-medium flex justify-end gap-3">
                                ...
                                @if($user->isSuperAdmin() || $user->can('supplier.delete'))
                                            <div class="hs-tooltip">



                                                <a href="{{ route('supplier.delete', $item) }}" data-fc-placement="bottom" class=" h-8 w-8 rounded-full bg-gray-200 flex justify-center items-center" >
                                ...
                                @if($user->isSuperAdmin() || $user->can('supplier.edit'))
                                       <div class="hs-tooltip">
                                                <a href="{{ route('supplier.edit', $item) }}" data-fc-placement="bottom" class=" h-8 w-8 rounded-full bg-gray-200 flex justify-center items-center" >
                                ...
                                @if($user->isSuperAdmin() || $user->can('supplier.mapping'))
                                    @if($item->api)
                                        <div class="hs-tooltip">
                                            <a href="{{ route('supplier.mapping', $item) }}"
                                               data-fc-placement="bottom" class=" h-8 w-8 rounded-full bg-gray-200 flex justify-center items-center" >
                                                <i class="material-symbols-rounded font-light text-2xl transition-all
                                                 group-hover:fill-1">Directions</i>
                                            </a>
                                            <span class="hs-tooltip-content hs-tooltip-shown:opacity-100 hs-tooltip-shown:visible opacity-0 transition-opacity inline-block absolute invisible z-10 py-1 px-2 bg-gray-900 text-xs font-medium text-white rounded shadow-sm"
                                                  role="tooltip">
                                                        {{ __('Mapping') }}
